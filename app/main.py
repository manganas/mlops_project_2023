...
app = FastAPI(
    ...
    description="Classify bird photos to their common name species.",
    version="0.1",
)


saved_path = "models"
label2id_path = "data/processed"
...
feature_extractor = AutoFeatureExtractor.from_pretrained(
    pretrained, cache_dir="models/feature_extractor"
)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

with open(label2id_path + "/id2label.pkl", "rb") as f:
...
def get_classes():
    response = {
        ...
        "number_of_classes": f"{len(id2label.keys())}",
    }
    return response
